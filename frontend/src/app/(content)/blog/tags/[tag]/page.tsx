/**
 * Blog Tag Detail Page - Vardhman Mills Frontend
 * 
 * Displays blog posts filtered by tag
 * 
 * @module app/(content)/blog/tags/[tag]/page
 */
...
import { useBlogsByTag, useBlogTags } from '@/lib/api/blog';
...
export default function BlogTagPage({ params }: TagPageProps) {
  ...
  // API Queries
  const { data: postsData, isLoading: postsLoading } = useBlogsByTag(
    tagSlug,
    { page, limit: 12 }
  );
  ...
  const currentTag: BlogTag | undefined = tagsData?.data?.items?.find(
    (tag: BlogTag) => tag.slug === tagSlug
  );
  ...
  return (
    <>
      <SEOHead
        title={`${currentTag?.name || 'Tag'} - Blog - ${APP_INFO.NAME}`}
        description={`Articles tagged with ${currentTag?.name || tagSlug}`}
        canonical={`${URLS.BASE}/blog/tags/${tagSlug}`}
      />
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  {currentTag?.postCount !== undefined && (
                    <Badge variant="secondary" className="text-lg">
                      {currentTag.postCount} Articles
                    </Badge>
                  )}
                </div>
              </div>
              ...
              <div>
                {postsLoading ? (
                  ...
                  <EmptyState
                    ...
                    description="There are no articles with this tag matching your filters."
                    action={{
                      label: 'Clear Filters',
